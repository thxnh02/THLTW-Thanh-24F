...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
    <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
      ...
      {user ? <div className="relative hidden sm:block"><button type="button" onClick={() => setAccountOpen((open) => !open)} className="inline-flex min-h-11 max-w-40 items-center gap-2 rounded-md bg-slate-100 px-3 text-sm font-semibold text-slate-900 hover:bg-slate-200" aria-expanded={accountOpen} aria-haspopup="menu"><span className="max-w-24 truncate">{user.name}</span><span aria-hidden="true">⌄</span></button>{accountOpen ? <div className="absolute right-0 mt-2 w-52 rounded-md border border-slate-200 bg-white p-2 shadow-lg" role="menu"><Link href="/account/profile" className="block rounded px-3 py-2 text-sm hover:bg-slate-100" role="menuitem">Tài khoản</Link><Link href="/account/orders" className="block rounded px-3 py-2 text-sm hover:bg-slate-100" role="menuitem">Đơn hàng</Link><Link href="/account/addresses" className="block rounded px-3 py-2 text-sm hover:bg-slate-100" role="menuitem">Địa chỉ</Link>{staffRoles.includes(user.role) ? <Link href="/admin" className="block rounded px-3 py-2 text-sm hover:bg-slate-100" role="menuitem">Quản trị</Link> : null}<button type="button" onClick={() => void logout()} className="mt-1 w-full rounded border-t border-slate-200 px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50" role="menuitem">Đăng xuất</button></div> : null}</div> : <Link href="/login" className="hidden min-h-11 items-center rounded-md bg-slate-950 px-3 text-sm font-semibold text-white hover:bg-teal-800 sm:inline-flex">Đăng nhập</Link>}
    </div>

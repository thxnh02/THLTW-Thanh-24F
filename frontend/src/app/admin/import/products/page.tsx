...
export default function AdminProductImportPage() {
  ...
  async function submit(event: SyntheticEvent, action: "preview" | "import") {
    ...
    try {
      ...
      setResult(payload);
      setMessage(action === "preview" ? "Da kiem tra file CSV." : "Da import file CSV.");
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : "Xu ly file that bai.");
    } finally {
      setProcessing(false);
    }
  }
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-3xl font-bold text-slate-950">Import san pham CSV</h1>
        ...
        <section className="mt-6 rounded-md border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 overflow-hidden rounded-md border border-slate-200">
            <table className="w-full border-collapse text-left text-sm">
              <thead className="bg-slate-100 text-slate-700"><tr><th className="p-3">Dong</th><th className="p-3">SKU</th><th className="p-3">Loi</th></tr></thead>
              <tbody>
                {result.rows.map((row) => (

  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-6">
        ...
        <input type="number" placeholder="Giá vốn" value={form.unit_cost} onChange={(event) => setForm({ ...form, unit_cost: event.target.value })} className="h-10 rounded-md border border-slate-300 px-3" />
        <input placeholder={form.type === "import" ? "Nhà cung cấp" : "Lý do"} value={form.type === "import" ? form.supplier : form.reason} onChange={(event) => setForm(form.type === "import" ? { ...form, supplier: event.target.value } : { ...form, reason: event.target.value })} className="h-10 rounded-md border border-slate-300 px-3" />
        <input placeholder="Ghi chú" value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} className="h-10 rounded-md border border-slate-300 px-3 lg:col-span-5" />
        <button className="rounded-md bg-slate-950 px-4 text-sm font-semibold text-white">Ghi phiếu</button>
      </form>
        ...
        <div className="mt-3 overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
          <table className="w-full border-collapse text-left text-sm">
            <thead className="bg-slate-100 text-slate-700">
              <tr><th className="p-3">Mã</th><th className="p-3">Loại</th><th className="p-3">Mặt hàng</th><th className="p-3">Ghi chú</th></tr>
            </thead>
            <tbody>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm">
          <table className="w-full border-collapse text-left text-sm">
            <thead className="bg-slate-100 text-slate-700">
              <tr><th className="p-3">SKU</th><th className="p-3">Thay đổi</th><th className="p-3">Sau đó</th><th className="p-3">Lý do</th><th className="p-3">Giá</th></tr>
            </thead>
            <tbody>

...
trait RendersOrderInvoice
{
    protected function renderInvoiceHtml(Order $order): string
    {
        ...
        return '<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>Hóa đơn '.$order->code.'</title><style>
...
<div class="header"><div><h1>Hóa đơn bán hàng</h1><p class="muted">'.e(config('app.name')).'</p></div><div class="right"><strong>'.e($order->code).'</strong><br><span class="muted">'.e($order->created_at?->format('d/m/Y H:i')).'</span></div></div>
<div class="box"><strong>Khách hàng</strong><br>'.e($order->customer_name).'<br>'.e($order->customer_email).' - '.e($order->customer_phone).'<br>'.e($order->shipping_address).'</div>
<table><thead><tr><th>Sản phẩm</th><th class="right">SL</th><th class="right">Đơn giá</th><th class="right">Thành tiền</th></tr></thead><tbody>'.$rows.'</tbody></table>
<table><tr><td>Tạm tính</td><td class="right">'.$this->money($order->subtotal).'</td></tr><tr><td>Giảm giá</td><td class="right">'.$this->money($order->discount_total).'</td></tr><tr><td>Phí vận chuyển</td><td class="right">'.$this->money($order->shipping_fee).'</td></tr><tr><td class="total">Tổng cộng</td><td class="right total">'.$this->money($order->grand_total).'</td></tr></table>
<p>Vận chuyển: '.e($order->shipping_method_name ?? 'Tiêu chuẩn').' - '.e($order->shipping_carrier ?? 'Đang cập nhật').' '.e($order->tracking_code ?? '').'</p><p>Thanh toán: '.e(strtoupper($order->payment_method)).' - '.e($order->payment_status).'</p><button class="print" onclick="window.print()">In / Lưu PDF</button></body></html>';
    }
